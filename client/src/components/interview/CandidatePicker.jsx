...
function CandidateCard({ entry, onSelect, index }) {
  ...
  return (
    <motion.button
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-6 text-left shadow-soft transition hover:border-brand-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"
    >
      <span className="pointer-events-none absolute inset-0 bg-gradient-to-br from-brand-400/0 via-fuchsia-400/0 to-amber-300/0 opacity-0 transition-opacity duration-500 group-hover:opacity-10" />
      ...
      <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4 text-xs">
        {typeof signals.commitDays === "number" && (
          <span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-slate-600">
            ...
            {signals.commitDays} active days
          </span>
        )}
        <span className="inline-flex items-center gap-1 rounded-md bg-slate-100 px-2 py-1 text-slate-600">
          <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" /> {passed}{" "}
          missions shown
        </span>
      </div>
